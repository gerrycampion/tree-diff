...
def _add_ngrams(arr, ngram_len, ngram_to_string, string_to_ngram):
    for val in arr:
...
def _find_closest_matches(text, base_ngram_to_string, string_to_ngram):
...
def _remove(lst, string_to_ngram, item):
    lst.remove(item)
...
def _intersections(string_to_ngram, base, compare, pairs, ngram_len):
...
def _match_lists(base, compare, pairs, ngram_len, steps):
    print(f"==========={steps}============")
